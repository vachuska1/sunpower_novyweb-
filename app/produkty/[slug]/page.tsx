...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!service) notFound();
  if (slug === "ostrovni-dum") return <IslandHouse />;
  return <><Header /><main className="detail-main">
    ...
    <section className="detail-gallery"><div className="container"><div><h2>Technologie v praxi</h2></div><div className="detail-gallery-images">{service.photos.map((photo, index) => <div key={photo}><Image src={photo} fill sizes="(max-width: 700px) 100vw, 50vw" alt={`${service.title} – realizace ${index + 1}`} /></div>)}</div></div></section>
