...
export default function RemoteProgrammingPage() {
  return <><Header /><main className="remote-page">
    ...
    <section className="remote-steps"><div className="container"><div className="section-head"><div><h2>Jak spolupráce funguje</h2></div><p>Jednotné označení komponent propojuje projekt, rozvaděč a práci technika na místě. Programování zajišťujeme vzdáleně.</p></div><div className="remote-step-grid">{steps.map(([number,title,text]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
    ...
    <section className="remote-gallery"><div className="container"><div className="section-head"><div><h2>Přehledné ovládání odkudkoliv</h2></div><p>Každá vizualizace vzniká podle konkrétní technologie a požadavků uživatele. Kliknutím ji můžete zobrazit v plné velikosti.</p></div><RegulationGallery /></div></section>
